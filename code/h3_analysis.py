...
OUTPUT_DIR = Path("../exploration_output/h3_analysis")
...
def load_em_data():
    """Load all cleaned participant files and extract ONLY Event-Middle (EM) frames."""
    ...
    for csv_file in csv_files:
        try:
            ...
            # Filter specifically for Event-Middle (EM) frames
            # Based on target_img filenames containing 'EM_T'
            em_trials = trial_data[trial_data['target_img'].astype(str).str.contains('EM_T', na=False)].copy()
            if em_trials.empty:
                continue

            em_trials['resp.corr'] = pd.to_numeric(em_trials['resp.corr'], errors='coerce')

            # Calculate participant-level accuracy specifically for EM frames
            mean_acc_em = em_trials['resp.corr'].mean() * 100

            all_data.append({
                'participant_id': participant_id,
                'condition': condition,
                'accuracy_em': mean_acc_em,
                'em_trial_count': len(em_trials)
            })
...
def perform_mann_whitney(data, variable, title):
    ...
    print(f"p-value = {p_value:.4f} {'(Significant difference detected)' if p_value < 0.05 else '(No significant difference)'}")
    ...
    plt.xlabel('Mean Accuracy on EM Frames (%)')
    plt.xlim(0, 110) # Using xlim because orient='h' flips the axes

    plt.annotate('Expected:\nNo Difference', xy=(0.85, 0.85), xycoords='axes fraction',
    ...
    plt.ylabel('Mean Accuracy on EM Frames (%)')
    plt.ylim(0, 100)
...
def main():
    print("Loading data and filtering for Event-Middle (EM) frames...")
    df = load_em_data()
    ...
    print(f"\nAverage EM trials analyzed per participant: {df['em_trial_count'].mean():.1f}")
